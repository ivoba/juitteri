 ...
 *
 * $Id: jquery.prettydate.js 6096 2009-01-12 14:12:04Z joern.zaefferer $
 *
...
(function() {
	...
	$.prettyDate = {
		...
		format : function(time) {
			var date = new Date((time || "")), diff = ($.prettyDate.now()
					.getTime() - date.getTime()) / 1000, day_diff = Math
					.floor(diff / 86400);
			...
			return day_diff == 0
					&& (diff < 60 && messages.now || diff < 120
							&& messages.minute || diff < 3600
							&& messages.minutes(Math.floor(diff / 60))
							|| diff < 7200 && messages.hour || diff < 86400
							&& messages.hours(Math.floor(diff / 3600)))
