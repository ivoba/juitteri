...
(function($) {
	...
	// YOU CAN CHANGE THE DYNAMIC VARS ON CALLING THE start method, see the
	// system.js for more information about it.

	numMSG = 20; // set the number of messages to be show
	containerDiv = "juitterContainer", // //Set a place holder DIV which will
	// receive the list of tweets example
	// <div id="juitterContainer"></div>
	loadMSG = "Loading messages...", // Loading message, if you want
	// to show an image, fill it
	// with "image/gif" and go to
	// the next variable to set
	// which image you want to use
	// on
	imgName = "loader.gif", // Loading image, to enable it, go to the
	// loadMSG var above and change it to
	// "image/gif"
	readMore = "Read it on Twitter", // read more message to be show
	// after the tweet content
	nameUser = "image" // insert "image" to show avatar of "text" to
	// show the name of the user that sent the tweet
	live: "live-20", // optional, disabled by default, the number after
	// "live-" indicates the time in seconds to wait before
	// request the Twitter API for updates, I do not
	// recommend to use less than 60 seconds.
	dateFormat = 'pretty'// pretty, locale, normal,
	// end of configuration

	// some global vars
	aURL = "";
	msgNb = 1;
	var mode, param, time, lang, dateFormat, contDiv, loadMSG, gifName, numMSG, readMore, fromID, ultID, filterWords;
	var running = false;
	// Twitter API Urls
	...
	$.Juitter = {
		...
		loading : function() {
			if (loadMSG == "image/gif") {
				$("<img></img>").attr('src', gifName).appendTo("#" + contDiv);
			} else
		...
		createURL : function() {
			...
			if (seachMult > 0)
				param = "&ors=" + param.replace(/,/g, "+");
			if (mode == "fromUser" && seachMult <= 0)
				url = apifUSER + param;
			else if (mode == "fromUser" && seachMult >= 0)
				url = apifMultipleUSER + param;
			else if (mode == "toUser" && seachMult <= 0)
				url = apitUSER + param;
			else if (mode == "toUser" && seachMult >= 0)
				url = apitMultipleUSER + param;
			else if (mode == "searchWord")
				url = apiSEARCH + param + jlg;
			url += "&rpp=" + numMSG;
			return url;
		},
		delRegister : function() {
			...
			if (msgNb >= numMSG) {
				$(".twittLI").each(function(o, elemLI) {
					if (o >= numMSG)
						$(this).hide("slow");
				});
			}
											...
											function(i, item) {
												if (e == 1
													...
													if (item.text != "undefined") {
														...
														if (fromID == "image")
															mHTML = "<a href='http://www.twitter.com/"
																	+ item.from_user
																	+ "'><img src='"
																	+ item.profile_image_url
																	+ "' alt='"
																	+ item.from_user
																	+ "' class='juitterAvatar' /></a> "
																	+ $.Juitter
																			.textFormat(tweet)
																	+ " -| <span class='time'>"
																	+ $.Juitter
			...
			if (filterWords) {
				...
				if (searchWords.length > 0) {
					...
					$.each(searchWords, function(i, item) {
						sW = item.split("->").length > 0 ? item.split("->")[0]
								: item;
		...
		textFormat : function(texto) {
			...
			texto = texto.replace(exp, "<a href='$1' class='extLink' "
					+ openLink + ">$1</a>");
